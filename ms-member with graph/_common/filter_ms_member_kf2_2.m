function output_member = filter_ms_member_kf2(filter, model, meas, truth)
% output_member = filter_ms_member_kf(model, meas, truth)
% INPUT:
%      model - structure that contains kinematic and observation params
%      meas  - structure that contains measurements
%      truth - structure that contains ground truth tracks
% OUTPUT:
%      output_member - structure that hold output estimates
% -------------------------------------------------------------------------
% Function that implements the multi-sensor multi-Bernoulli filter proposed
% in [1]. The state models are linear and Gaussian, hence the 
% implementation is based on the Kalman filter. 
% -------------------------------------------------------------------------
% [1] A. Saucan, M. Coates, and M. Rabbat,  "A multi-sensor multi-Bernoulli 
% filter"  2016, arXiv:1609.05108 [stat.ME]
%
% Contact: augustin.saucan@mail.mcgill.ca

%output variables
output_member.state_X_hat = cell(meas.K,1);   % inferred target states
output_member.graph_G_hat = cell(meas.K,1);   % estimated graph
...
for k = 1:meas.K
    ...
    MS_MeMBer_predict = predict1(MS_MeMBer_update, model, Graph{k-1});   
    end
    ...
    % State estimation
    [output_member.state_X_hat{k}, output_member.state_n_hat(k), output_member.graph_G_hat{k}, ind] = extract_estimates(MS_MeMBer_update);
    ...
    % generate graphs 对确定状态建立图结构，未确定状态继续单目标操作
    [Graph{k}, MS_MeMBer_update] = gen_groups(MS_MeMBer_update, ind, filter);
end
...
for i = 1:model.birth.J_gamma
    ...
    MS_MeMBer_predict.G(MS_MeMBer_predict.M + i) = 0;
end
...
MS_MeMBer_predict.r = MS_MeMBer_update.r * model.kin.pS;
if MS_MeMBer_update.M
    [MS_MeMBer_predict.m, MS_MeMBer_predict.P] = kalman_predict_multiple2(model, MS_MeMBer_update.m, MS_MeMBer_update.P, MS_MeMBer_update.G, Gr);
end
...
if MS_MeMBer_predict.M
   ...
   MS_MeMBer_predict.r = MS_MeMBer_predict.r(sorted);
    ...
    for i = 1:MS_MeMBer_predict.M
        sorted_m(:,i) = MS_MeMBer_predict.m(:,sorted(i));
        sorted_P(:,:,i) = MS_MeMBer_predict.P(:,:,sorted(i));
        sorted_G(:,i) = MS_MeMBer_predict.G(:,sorted(i));
    end
    MS_MeMBer_predict.m = sorted_m;
    MS_MeMBer_predict.P = sorted_P;  
    MS_MeMBer_predict.G = sorted_G;
end
...
if empty_data
    ...
    MS_MeMBer_update.r = gamma * MS_MeMBer_predict.r ./ (1 - MS_MeMBer_predict.r +  MS_MeMBer_predict.r * gamma);
        ...
        %% S-D assignment find best partitions
    P_partition=myfind_partitions(model, filter, MS_MeMBer_predict, Z);
    sub=P_partition.sub;
    m_paths=P_partition.m;
    P_paths=P_partition.P;
    alpha_P=P_partition.alpha_P;
    index = 0;                                                             % counter for updated Bernoulli components 
    r_kk = [];
    
    for j = 1:MS_MeMBer_predict.M   
        dw_mat=zeros(length(sub),model.obs.nb_sensors);
        for i=1:length(sub)
            dw_mat(i,:)=sub{i}(j,2:end);
        end
  
        [unique_subsets_j,IA,IC] = unique(dw_mat,'rows');                     % consider only the unique subsets
        
        nb_unique_subsets_j = size(unique_subsets_j,1);
               
        for  i = 1:nb_unique_subsets_j                                     % an updated Bernoulli component is created for each unique subset 
            index = index + 1; 
            
            if sum(unique_subsets_j(i,:)) ~= 0                                     % if subset is not the empty subset \emptyset_{1:s}
                r_kk(index) = sum( alpha_P( IC ==  i ) );  % add the weight of all partitions that assign the subset unique_subsets_j(i) to the j-th Bernoulli
                ...
                MS_MeMBer_update.m(:,index) = m_paths{IA(i)}(:,j);    % updated mean and covariance                 
                MS_MeMBer_update.P(:,:,index) = P_paths{IA(i)}(:,:,j);
                MS_MeMBer_update.G(index) = MS_MeMBer_predict.G(j);
            else                                                                     % if subset is the empty subset => legacy component, use the predicted Bernoulli and only update the existece probability
                r_kk(index) = sum( alpha_P( IC ==  i  ) ) * gamma * MS_MeMBer_predict.r(j) ./ (1 - MS_MeMBer_predict.r(j) +  MS_MeMBer_predict.r(j) * gamma);
...
if member_in.M > N_cap
    ...
    member_out.G = member_in.G(idxkeep);
...
function [X_hat, N_hat, G_hat, ind] = extract_estimates(member_in)
