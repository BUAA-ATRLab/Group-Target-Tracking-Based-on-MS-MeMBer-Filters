...
m_update = zeros(model.kin.x_dim, zlength*plength);
P_update = zeros(model.kin.x_dim, model.kin.x_dim, plength);

for idxp=1:plength
        [qz_temp, m_temp, P_temp] = kalman_update_single(z, model.obs.H, model.obs.R, m(:,idxp), P(:,:,idxp));
        qz_update(idxp,:)  = qz_temp;
        
        m_update(:,idxp + (0:zlength-1)*plength) = m_temp;
...
qz_temp = exp(-0.5*size(z,1)*log(2*pi) - 0.5*log(det_S) - 0.5*dot(z-repmat(mu,[1 size(z,2)]),iS*(z-repmat(mu,[1 size(z,2)])),1))';
